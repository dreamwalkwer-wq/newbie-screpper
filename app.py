...
def log(msg: str):
    """Лог в стиле твоих print, но сразу в интерфейс."""
...
def deduplicate(df):
    ...
    result = result.drop(columns=["review_norm", "author_norm", "title_norm", "date_norm"], errors="ignore")
    return result, removed

# ---------------------------
# Mojibake fix (крякозябры)
# ---------------------------
...
def fetch_appstore_reviews(app_id, country="us", how_many=100, language_filter=None):
    ...
    app_id, country, how_many, language_filter = validate_inputs(app_id, country, how_many, language_filter)
...
st.markdown(
    ...
    "Сервис использует Apple RSS feed, может вернуть меньше отзывов, чем вы запросили."
)
...
with col3:
    how_many_input = st.number_input("Сколько отзывов собрать", min_value=1, max_value=1000, value=100)
with col4:
    language_filter_input = st.text_input("Фильтр по языку (например, 'ru', 'en', пусто = без фильтра)", value="")
...
if run_button:
    try:
        ...
        # XLSX download
        xlsx_buffer = pd.ExcelWriter(
            f"/tmp/appstore_reviews_{app_id_input}_{country_input}_{language_filter_input or 'all'}.xlsx",
            engine="openpyxl"
        )
        df_result.to_excel(xlsx_buffer, index=False)
        xlsx_buffer.close()

        with open(xlsx_buffer.path, "rb") as f:
            xlsx_bytes = f.read()
